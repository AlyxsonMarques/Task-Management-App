...
class teamRepository {
  ...
  async createTeam(team) {
    const result = await this.database.query(
      `INSERT INTO team (name, members, administrators) VALUES ($1, $2, $3)`,
  ...
  async getAllTeams() {
    const result = await this.database.query(`SELECT * FROM team`)
  ...
  async updateTeam(team) {
    const result = await this.database.query(
      `UPDATE team SET name = $1, members = $2, administrators = $3 WHERE id = $4`,
